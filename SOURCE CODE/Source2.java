...
import java.net.*;
/**
 * Summary description for Source2
 *
 */
public class Source2 extends JFrame
{
	...
	String destination;
	int limit;
	String a[]={"Select","R-106","R-107","R-108","I-109"};
	int len;
	int packets;
	int rem;
	...
	private void initializeComponent()
	{
		...
		// Source2
		//
		this.setTitle("Source2 - extends JFrame");
	...
	private void jButton2_actionPerformed(ActionEvent e)
	{
		...
		if(flag==0)
		{
			...
			{
				JOptionPane.showMessageDialog(null,"This Is From R-106");
				dest=setDest(a[1]);
				sendData("localhost",333,dest);

			}
			else if(destination.equalsIgnoreCase(a[2]))
			{
				JOptionPane.showMessageDialog(null,"This Is From R-107");
				dest=setDest(a[2]);
				sendData("localhost",111,dest);
			}
			else if(destination.equalsIgnoreCase(a[3]))
			{
				JOptionPane.showMessageDialog(null,"This Is From R-108");
				dest=setDest(a[3]);
				sendData("localhost",333,dest);
			...
			{
				JOptionPane.showMessageDialog(null,"This Is From I-109");
				dest=setDest(a[4]);
				sendData("localhost",333,dest);
			}
		}
